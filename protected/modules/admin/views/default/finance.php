...
<div class="wrapper">
  <div class="left-nav">
    <div id="side-nav">
      <ul id="nav">
            <?php  DefaultController::printMenu(5); ?>
       </ul>
    </div>
  </div>
  <div class="page-content">
    <div class="content container">
      <div class="row">
       <div class="col-lg-12">
            <div class="col-lg-6"><h2 class="page-title">Finance</h2></div>
...
<div class="example_alt_pagination">
  ...
  <div id="demo">
      <div class="row" style="padding:20px; margin:0;margin-bottom: 20px;background: rgba(0,0,0,0.2); border-radius: 6px;">
            ...
            <div class="col-lg-12">
                <div class="col-lg-6">
                    <a onclick="getTableData();" class="btn btn-s-md btn-success btn-sm" href="#" >Update</a>
                </div>
                
                <div class="col-lg-6" align="right">
                    <button id="btnnewPayin" data-backdrop="false" data-target="#myModal" data-toggle="modal" class="btn btn-s-md  btn-default btn-sm" type="button">New Payin</button>
                </div>
            </div>
      </div>
    <table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
      <thead>
            <tr>
                ...
                <th class="hidden-xs" style="width:140px">Statement</th>
      ...
      <tbody id="tableContainer" style="font-size:12px"></tbody>
    ...
    <input id="payinOption" hidden value="">
    <input id="inputID" hidden value=""/>
              ...
              <div class="modal-content">
                ...
                <div class="modal-body" >
                    <table style="width:520px;margin-top:10px;">
                        ...
                        <tr style="height: <?php echo $tableHeight.'px' ?>;">
                          ...
                          <td style="font-weight: bold"><input class="typeahead" id="inputPayRef" onblur="updateProgress();" type="text" placeholder="" style="width:97%;background-color: white;color:black;margin:3px;border: 1px solid #dee4ea;text-align:right"></td> 
                          <td></td>
                        </tr>
                        <tr style="height: <?php echo $tableHeight.'px' ?>;">
                          <td>From account</td>
                          <td style="font-weight: bold"><input id="inputFrom" type="text" placeholder=""  onkeyup="checkInputVal('inputFrom')" style="width:97%;background-color: white;color:black;margin:3px;border: 1px solid #dee4ea;text-align:right"></td> 
                          <td></td>
                        </tr>
                        <tr style="height: <?php echo $tableHeight.'px' ?>;">
                          <td>To account</td>
                ...
                <div class="modal-footer">
                    ...
                    <div id="modalAskSure" hidden>
                        ...
                        <button class="btn btn-success" type="button" onclick='saveNewPayin(document.getElementById("payinOption").value);'>Yes</button>
                    </div>
                </div>
              </div>
              <!-- /.modal-content --> 
    ...
    <div  aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" class="modal" id="delModal">
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              <button onclick="closeModal();" aria-hidden="true" data-dismiss="modal" class="close" type="button">x</button>
              <h4 id="delModalLabel" class="modal-title">Payout confirmation</h4>
            </div>
            <div class="modal-body" >
                <p style="font-size:16px;text-align:center;color:red;font-weight: 300;margin-top:24px"> Are you sure to delete this payin?</p>
                
            </div>
            <div class="modal-footer">
                <div id="modalAskSure" >
                    <button class="btn btn-danger" type="button" onclick="closeModal();">No</button>
                    <button class="btn btn-success" type="button" onclick="delPayinFinal();">Yes</button>
                </div>
            </div>
           </div>
          <!-- /.modal-content --> 
...
<script type="text/javascript" charset="utf-8">
   ...
   function saveNewPayin(type) {
        var oldID   = document.getElementById("inputID").value;
        var a       = document.getElementById("inputStaNo").value;
        ...
        if (checkInputValues()) {
            var sumAll = a+'|'+b+'|'+c+'|'+d+'|'+e+'|'+f+'|'+g*100+"|"+h+"|"+type+"|"+oldID;
            $.ajax({
   ...
   function resetFields(){
    ...
    function delPayin(id) {
        
        $('#delModal').fadeIn('fast');
        $('#delModalLabel').text('Payin delete confirmation #'+id);
    ...
    function delPayinFinal() {
    var inputID = document.getElementById('inputID').value;
        $.ajax({
            url: "/admin/default/delPayin",
            type: "post",
            dataType: 'html',
            data: "data="+inputID,
            success: function(data){
    ...
    $('#btnnewPayin').on('click', function() {
        document.getElementById("payinOption").value = '0';
  
    });
    
    $(document).on("mousedown", ".editBTN", function() {
        document.getElementById("payinOption").value = '1';
        document.getElementById("inputID").value = $(this).attr('data-id');
        
        
        $.ajax({
                 url: "/admin/default/getPayinById",
                 type: "post",
                 dataType: 'html',
                 data: "data="+$(this).attr('data-id'),
                 success: function(data){
                   var dataS = data.split('|');
                        $('#inputStaNo').val(dataS[0]);
                        $('#inputPayRef').val(dataS[1]);
                        $('#inputFrom').val(dataS[2]);
                        $('#inputTo').val(dataS[3]);
                        $('#inputAmount').val(dataS[4]);
                 },
                 error:function(data){
                     alert('error');
                 }
...
$('#inputPayRef').typeahead({
    hint: true,
    highlight: true,
    minLength: 4, 
    valueKey: 'ref',
    remote: {
        url:'/admin/default/getPRef/id/%QUERY',
        dataType:'json'
    },
    template:  '<p style="font-weight:bold">{{ref}}</p><table style="width:200px"><tr><td style="font-size:12px;font-weight:300">{{buyer}}</td><td style="text-align:right;font-size:12px;font-weight:300">{{amount}}kr</td></tr></table>',
    engine: Hogan
  });
